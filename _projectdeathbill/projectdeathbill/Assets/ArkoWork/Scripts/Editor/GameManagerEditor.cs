...
public class GameManagerEditor : Editor {
	...
	public override void OnInspectorGUI ()
	{
		...
		for(int i=0;i<gmManager.workList.Count;i++)
		{
			...
			/*if(wi.work!=null)
			{
				SerializedObject obj = new SerializedObject(wi.work);
				SerializedProperty pop = obj.GetIterator();
				bool enterChildren = true;

				while(pop.NextVisible(enterChildren))
				{
					EditorGUI.BeginDisabledGroup("m_Script" == pop.propertyPath);
					EditorGUILayout.PropertyField(pop,true, new GUILayoutOption[0]);
